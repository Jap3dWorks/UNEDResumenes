...
| Transporte      | segmento (TCP) / datagrama de usuario (UDP) | [c.trasporte][mensaje]                  | Número de puerto                     |
...
- Según el analisis de Fourier, cualquier señal compuesta es una combinación de ondas senoidales simples
...
- *Transmisión en banda ancha*
      ...
      - Implica cambiar la señal digital a analógica.
            - Permite el uso de un *canal paso-banda*.
                  - Canal con un ancho de banda que NO empieza en 0 Hz.
...
- Ocurre en una señal compuesta donde cada señal componente tiene su propia propagación.
...
- *Ancho banda en bps*, número de bits por segundo de un canal (tambiés se conoce como tasa de bits).
- Mayor ancho de banda en hercios mayor ancho de banda en bps.

*** Tasa de transferencia relativa (throughput)
- bps reales de un canal, siempre es inferior al ancho de banda del canal (tasa de bits).
- Se de a congestiones o otras causas.
...
- *Elemento señal frente a elemento de datos*
      ...
      - *Elemento señal*, unidad más corta en el tiempo de una señal digital.
            - Un elemento señal representa el cambio de la señal.
      - Los elementos dato son transportados y los elementos señal son portadores.
...
- *Tasa de datos frente a tasa de señales*
      ...
      - $S_{ave}=c \times N \times \frac{1}{r}\ baudios$
            ...
            - $c$, factor de caso, varia en cada caso (suele ser 1/2).
            - Es el caso medio, las señal no tiene porque estar cambiando en cada intérvalo de tiempo.
...
- *2B1Q*, dos binario 1 cuaternario.
      - Codifica dos bits en una señal de cuatro niveles.
      - $r=2$, $S_{ave} = \frac{N}{4}$
...
- *8B6T*
      ...
      - $r=8/6$, $S_{ave} = 1/2 \times N \times 6/8$
      - $B_{ave} = 3N/4$
- *4D-PAM5*
...
- $n_b \times B_{analog}$ tasa de bits.
...
- *Procedimiento*
      ...
      - *Señal de interferencia*, se envía una señal corta en caso que otras estaciones no hayan detectado la colisión.
- *Nivel de energía*
      - Cero, canal libre.
...
**** CSMA/CA
:PROPERTIES:
:ID:       5f58767c-28b3-4ffe-8988-9e5f24ad8f99
:END:
...
- Linea de tiempo de intercambio de trama
      ...
      4. El destino, al acabar la retransmisiónm, después de esperar SIFS envía un ACK.
...
- *Selección*
      - El nodo primario debe alertar al nodo secundario de su deseo de transmitir (trama de selección SEL).
...
*** Control de acceso
- [[id:5f58767c-28b3-4ffe-8988-9e5f24ad8f99][CSMA/CA]]
...
- Tipos de estaciones
      ...
      - Estación con *movilidad con transición ESS*
...
**** Conmutador del nivel de enlace (SWITCH)
...
- *Paquete de reconocimiento*
      1. El destino responde con la etiqueta que ha asignado.
             - El enrutador conectado al destino usará esta etiqueta para rellenar su tabla de conexión y saber que puerto de salida corresponde a esta conexión.
...
- *Control de la congestión en bucle abierto (prevención)*
      ...
      - *Política de reconocimiento*, por ejemplo el acuse de recibo de N paquetes a la vez.
...
- Un equipo necesita saber, el servidor DHCP los puede proporcionar.
      - Su dirección IP
      - prefijo de la red
      - dirección del enrutador (gateway)
...
- *Proceso*
      ...
      4. El servidor responde al cliente con DHCPACK si la dirección ofrecida es válida.
             - En caso contrario responde con DHCPNACK.
...
- Require de enrutadores con tabla de enrutamiento
...
- *Agregador de direcciones*
      - Todas las subredes que conectan con un enrutador A pueden tener una red común.
            - Redes en bloque.
      - Esto simplifica a los enrutadores externos (B) la tabla de enrutación.
...
- Tasa Bits Muestreo $= 2 \cdot B \cdot log_2(L)\ bps$
- $B_{BASK} = (1 + d)S\ Hz$
- $B_{BFSK} = (1+d)S + 2\Delta_f\ Hz$
