...
- *Forwarding* o *adelantamiento*. Conexión entre etapas *EX*, haciendo posible realizar un cálculo con el resultado de la instrucción anterior sin esperar a la etapa *WB*.
- *Interbloqueo*, La instrucción se detiene a la espera del resultado que necesita, se implementa con elementos hardware.
...
- *Branch target Buffer*, Similar a BTAC pero además se almacena un conjunto de bits con el historial de aciertos (campo BH).
      - El historial sirve para realizar un reemplazo si es preciso (posibilidad de no efectiva).
...
- *PHT*, Pattern history table.
      ...
      - Cada entrada tiene un contados de saturación de 2 bits.
            - Ac-tua como el predictor bimodal.
...
- *ARF* entradas
      ...
      - *Ocupado*, indica si ha sido renombrado.
      - *Índice*, apunta a la entrada del RRF.
- *RRF* entradas
      - *Datos*, contiene el valor.
      - *Válido*, El valor del RRF se puede usar.
      - *Ocupado*, inica si esta siendo usado por instrucciones.
...
- *ARF* entradas
      ...
      - *Ocupado*, indica si ha sido renombrado.
- *RRF* entradas
      - *Destino*, identificador del registro del *ARF*
      - *Datos*, contiene el valor.
      - *Válido*, El valor del RRF se puede usar.
      - *Ocupado*, inica si esta siendo usado por instrucciones.
      - *último*, indica que es el último registro de renombramiento asignado al registro destino.
            - Puede haberse renombrado varias veces.
            - Una instrucción posterior puede saber que registro de renombramiento es la que debe usar para realizar un reemplazo.

**** Organización del RRF como parte del buffer de reordenamiento
- el *RRF* forma parte del buffer de reordenamiento.
- *ARF* entradas
      - *Datos*, contiene el valor del registro.
      - *Ocupado*, ha sido renombrado.
      - *Índice*, puntero a una de las entradas del buffer de reordenamiento.
- *Buffer de reordenamiento RRF*
      - *Datos*, contiene el valor del registro.
      - *Válido*, El valor del registro se puede usar.
      - *Ocupado*, Indica si la entrada del buffer es válida.
      - otros campos propios del buffer de reordenamiento.
- Obliga a añadir el campo datos aunque se usen instrucciones que no provocan un dato de salida.
...
- *if-conversion*, técnica de cambiar instrucciones if por predicados.
...
- *Solapamiento*, Entre convoys pueden usar las unidades funcionales uno justo detrás del otro.
      - Si no hay solapamiento un convoy debe esperar a que el convoy anterior acabe
        completamente el uso del módulo.
...
** Rendimiento de un bucle vectorizado
- Un bucle muy largo se puede resolver con operaciones vectoriales (compilador strip mining).
- Se divide en secciones de MVL (max vector length) elementos.
      - Esto conlleva la existencia de un bucle exterior y uno interior.
- $T_{base}$, tiempo que consumen las instrucciones escalares de preparación antes de abordar el bucle exterior.
- $T_{bucle}$, Costes derivados de las operaciones de seccionamiento del bucle exterior.
- $T_{arranque}$, La suma de los tiempos de arranque visibles.
- $T_{elemento}$, número de convoys
- $T_{n} = T_{base} + \lceil n/MVL \rceil \times (T_{bucle} + T_{arranque}) + n \times T_{elemento}$

- $R_{\infty} = \lim_{n \to \infty} \left( \frac{Operaciones\ Vectoriales\ \times\ n}{T_n} \right)$
      - FLOP por ciclo de reloj.
      - FLOPS multiplicar por la frecuencia del reloj.

- Si hay solapamiento entre convoys
...
- Memoria compartida a través de la red de interconexión (pueden ser varios módulos).
      - *UMA*, Uniform memory access, tiempo de acceso a memoria es el mismo para cualquier palabra desde cualquier procesador.
...
- *Redes Multietapa*, Se componen de etapas (elementos lógicos) conectadas con otras etapas mediante conmutadores.
      ...
      - *Red Baseline*, Se construye por bloques conectando las salidas del los conmutadores $n$ con las entradas de los conmutadores $n+1$.
            ...
            - La permutación se hace según los dígitos del destino, de izq a derecha.
                  - dígito 1 se coge la conexión inferior al permutador.
                  - dígito 0 se coge la conexión superior al permutador.
                    [[file:images/RedBaseline.png]]
      - *Red Butterfly*,
