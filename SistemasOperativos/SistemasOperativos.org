...
- Según el número de programas en memoria
      ...
      - *Multiacceso*, permite el acceso a través de dos o más terminales.
...
- Según los requisitos temporales de los programas a ejecutar
      ...
      - *Sistemas operativos de tiempo compartido o sistemas interactivos*, multiusuario con multiprogramación.
            - El usuario cree ser el único usando el computador.
...
- Según la finalidad del computador
      ...
      - *Sistemas operativos integrados*, un reproductor mp3
- Otros
...
- *Llamadas al sistema*, interfaz que pone el SO a disposición de los programas, para usar funcionalidades del núcleo.
      ...
      - El paso de parámetros y/o obtención de resultados puede hacerse con registros, memoria localizada o almacenarlos en una pila dedicada.
...
- También se conoce como *imagen del proceso o espacio de direcciones de usuario*.
- Regiones
      - *Región de código*, corresponde a la región de código del archivo ejecutable.
      - *Región de datos*, corresponde a la región de datos del archivo ejecutable.
            - *datos inicializados*, tamaño fijo.
            - *datos no inicializados*, puede variar el tamaño.
      - *Región de pila*, Se crea automáticamente y el tamaño es ajustado dinámicamente en tiempo de ejecución.
            - Compuesta por marcos de pila lógicos.
            - Un marco se añade al proceso cuando se llama a una función y se quita cuando se sale.
                  - Cada marco alamcena el contador de programa y el puntero de pila anteriores
                        - Volver a la ejecución llamante de la función al salir.
- Se reserva algo de espacio adyacente a la región de pila ya la región de datos no inicializados porque pueden crecer.

** Estados de un proceso
- *Nuevo*, Tiene asignadas algunas estructuras de datos pero aún no está cargado en memoria principal.
- *Preparado*, Listo para ser ejecutado cuando el planificador lo considere.
- *Ejecutándose*, haciendo uso del procesador
- *Bloqueado*, Esperar a un determinado evento, por ejemplo la finalización de operaciones de E/S.
- *Terminado*, Ha finalizado su ejecución.
- 
  [[file:images/TransicionProcesoEstados.png]]
...
- Colas generales a las que pertenece un proceso
      - *Cola de procesos en el estado preparado*, procesos que desean acceder al procesador.
      - *Cola de procesos en el estado preparado en memoria secundária*, Procesos en esperar de voler a la memoria principal y en estado preparado.
      - *Cola de procesos en el estado bloqueado*, A la espera de poder volver a la cola de preparados tan pronto ocurra el evento por el que están bloqueados.
      - *Cola de procesos en el estado bloqueado en memoria secundaria*, Esperan para volver al estado preparado en memoria secundaria cuando ocurra el evento que esperan.
      - *Cola de trabajos por lotes o cola de entrada*, En memoria secundaria, almacena los nuevos trabajos que llegan al SO por lotes.

*** Planificador a corto plazo
- Que proceso en la cola de preparados será el siguiente en acceder al procesador.
- Componentes lógios
      - *Encolador*, Se encarga de incluir a los procesos en la cola de preparados y actualizar las estructuras de datos necesarias.
      - *Conmutador de contexto*, Guarda el contexto de un proceso para poder reanudarlo más tarde.
      - *Distribuidor*, Se encarga de seleccionar que proceso de la cola de procesos de acuerdo con un algoritmo de planificación.
            - Para que pueda tomar el control de la cpu.

*** Planificador a medio plazo
- Decide que procesos pasan de memoria principal a memoria secundaria.

*** Planificación a largo plazo
- Qué trabajo de la cola de *trabajo por lotes* pasa a ser ejecutado.
- *Procesos limitados por CPU*, mayor parte del tiempo están haciendo uso de la cpu.
- *Procesos limitados por E/S*, mayor parte del tiempo están esperando por operaciones de E/S.
- Se encarga de hacer una mezcla adecuada de trabajos limitados por cpu y limitados por E/S.

** Criterios generales considerador en la planificación de procesos
- *Equidad*, Procesos con requisitos similares uso de los recursos similares.
- *Previsibilidad*, Trabajos con características similares deben ejecutarse en tiempos similares.
- *Uso equilibrado de los recursos*, Intentar mantener ocupados todos los recursos del ordenador.
- *Proporcionalidad*, Peticiones percibida como sencillas deben emplear menor tiempo de respuesta.

** Criterios considerados en la planificación del procesador
- *Utilización del procesador o eficacia*, porcentaje de tiempo con el procesador en activo.
- *Productividad o rendimiento*, Trabajos completados por unidad de tiempo.
- *Tiempo de entrega*, tiempo de retorno, tiempo medio desde que se lanza un proceso hasta que finaliza.
- *Tiempo de espera*, Suma del tiempo de un proceso en colas de espera.
- *Tiempo de respuesta*, Tiempo desde que un usuario manda una orden en el terminal hasta que recibe una respuesta.
- *Plazo de finalización*, Tiempo máximo que tiene un proceso para ser completado (sistemas de tiempo real).
...
** Mensajes
- sockets y buzones.
- Soportado por sistemas centralizados y distribuidos.
- operaciones básicas
      - *send(destino, mensaje)*, proceso emisor.
            - *mensaje*,dirección de memoria con los datos a enviar.
      - *receive(fuente, mensaje)*, proceso receptor.
            - *mensaje*, dirección en memoria donde colocar los datos.

*** Comunicación directa
- El proceso emisor especifica explícitamente el proceso receptor (por ejemplo con el pid).
- Por su parte el receptor especifica explícitamente el proceso emisor (pid).
- Otra posibilidad es *receive(ident, mensaje*).
      - Siendo *ident* un parámetro de salida con el cual se conocerá el proceso emisor.
- Enlaces seguros entre procesos, peroa a veces no se puede saber el identificador de el proceso receptor o emisor.

*** Comunicación indirecta
- *send(buzon, mensaje)*, 
- *receive(buzon, mensaje)*
- Esquemas de difusión de mensajes
      - un emisor - un receptor
      - un emisor - varios receptores
      - varios emisores - un receptor
      - varios emisores - varios receptores
- Buzón propiedad de un proceso.
      - Solo el proceso creador tiene acceso.
      - Se implementa en el espacio de direcciones del proceso.
      - Desaparece con el proceso
- Buzón propiedad del SO
      - Se implementa en el espacio del núcleo
      - Un proceso lo crea con llamadas al sistema, se considera el propietario, inicialmente solo el propietario tiene acceso para recibir mensajes.
      - El propietario puede compartir el privilegio de leer del buzón.
      - El SO implementa llamadas para crear y eliminar buzones.

*** Esquemas de sincronización
- *Send con bloqueo*, bloqueado hasta que el mensaje sea recibido por el receptor.
- *Send sin bloqueo*, envía el mensaje y continua.
- *Receive con bloqueo*, el receptor se bloquea a la espera de recibir un mensaje.
- *Receive sin bloqueo*, obtiene un mensaje válido o nulo y continua su ejecución.
- Combinaciones que se posibilitan
      - Envío y recepción sin bloqueo.
      - *Envío sin bloqueo y recepción con bloqueo*, de los más usados.
      - Envío con bloqueo y recepción sin bloqueo.
      - Envío y recepción con bloqueo, se conoce como cita entre emisor y receptor.

*** Formato y almacenamiento de los mensajes
- Formato de un mensaje
      - *Cabecera del mensaje*, Información de control.
      - *Cuerpo del mensaje*, El contenido del mensaje propiamente.
- Capacidad de los buzones
      - *Mecanismo de mensajes con buffer*, Se permite almacenar temporalmente un número finito de mensajes.
            - Cada vez que se lee un mensaje se borra.
            - Si la cola está llena el proceso emisor se bloquea.
      - *Mecanismo de mensajes sin buffer*, Las colas de los mensajes son de capacidad nula.
...
* Interbloqueos
** Introducción
- Recursos hardware y recursos software (archivos, semaforos)
...
*** Denegación de asignación de recursos: Algoritmo del banquero
- Se hace una simulación de la asignación de los recursos.
      - Estado inicial: $S_k = \{N,A,R_E, R_D\}$
      - Estado ficticio: $S' = \{N',A',R_E,R'_E\}$
      - Si $S'$ es seguro $S_{k+1}=S'$
- Ante una petición de recursos se comprueba si la asignación lleva a un interbloqueo.
      - Si es así se denega la petición y el estado se bloquea hasta que se pueda aceptar la petición.
- Si el estado es correcto si procede con la petición.
...
- Busca la página más tiempo cargada en memoria que no ha sido cargada recientemente.
      - variante de *FIFO*.
- Lás páginas referenciadas tienen un bit de referencia que empieza en 0 ($r=0$).
      - Si la página es referenciada posteriormente $r=1$.
...
- *Subsistema de E/S*, Componente del sistema operativo que se encarga de la gestión de E/S.
  
** Peticiones de E/S
- Pueden ser peticiones de un proceso o del propio SO.
- Procesos o demonios solicitan la petición al SO.

** Capas del núcleo de un SO encargadas de la E/S
*** Subsistema de E/S
- Los procesos y demonios se comunican con esta capa.
- Tareas comunes e independientes de los dispositivos de E/S.
      - *Asignación y liberación de dispositivos dedicados*
            - Como una impresora, que solo puede ser usada por un proceso a la vez (dispositivo dedicado).
      - *Bloqueo de procesos que solicitan una operación de E/S*
            - Depende de si la llamada es en modo bloqueante.
      - *Planificación de la E/S*
            - Algoritmos de planificación.
      - *Invocación del driver de dispositivo apropiado*
            - Pieza que permite la comunicació entre el subsitema de E/S y el controlador del dispositivo.
            - Así cada dispositivo tiene su controlador propio pero el SO no es consciente de ello.
      - *Almacenamiento temporal de datos de E/S o buffering*
            - Creación y manejo de buffers
      - *Proporcionar un tamaño de bloque uniforme a los niveles superiores de software*
            - Así las capas superiores puede trabajar con un tamaño de bloque lógico único.
      - *Gestión de los errores producidos en una operación de E/S*
            - Decidir las acciones ante *errores de programación* y/o *errores en el dispositivo*.
- Proporciona la interfaz que los drivers deben implementar.
      - interfaz uniforme, uniforme a todos los dispositivos sea cual sea su naturaleza.
      - Interfaz no uniforme, según la naturaleza del dispositivo.

*** Drivers de dispositivos
- Permite a un SO controlar un determinado tipo de dispositivo de E/S
      - Sin conocer los detalles del dispositivo en concreto.
      - Facilitados por los fabricantes de dispositivos para cada SO.
            - Si el subsistema de E/S lo permite puede invocar procedimientos del núcleo.
                  - Si la llamada del subsistema que implementa el driver es en modo núcleo.
      - Acciones comunes
            - *Comprobar que los parámetros de la llamada son correctos*
            - *Traducir los parámetros de una función a parámetros específicos del dispositivo*
            - *Comprobar si el dispositivo de E/S está ocupado*
            - *Generar un conjunto de órdenes a partir de la llamada del subsistema de E/S*
                  - A veces se traduce en diferentes pasos en el controlador.
            - *Esperar a que el controlador ejecute las órdenes*
            - *Comprobar que no hay errores*
            - *Examinar la cola de peticiones de E/S pendientes*
                  - Si no hay el driver se bloquea en espera de nuevas peticiones.

*** Manejadores de las interrupciones
- Cuando un dispositivo de E/S finaliza una operación el controlador que lo supervisa genera una interrupción.
- Parte del SO muy dependientes del Hardware.
- Alta prioridad de ejecución
- Interrupciones vectorizadas
      - Solo si el hardware lo implementa.
      - Cada interrupción suministra un número de interrupción (*número del vector de interrupción*).
            - El número anterior sirve como entrada en *la tabla de vectores de interrupción*.
      - En cada entrada de *La tabla de vectores de interrupción* se encuentra un puntero al inicio del manejador de ese tipo de interrupción.

** Buffering
*** Estratégias
- *Buffering con buffer único*, Datos se transfieren al buffer posteriormente al buffer del espacio del proceso.
      - Permite al proceso consumidor procesar datos mientras se rellena el buffer.
- *Buffering con dos bufers*.
      - Se asignan dos buffers en una operación de E/S.
      - Si el primer buffer se llena se pasa a rellenar el segundo.
            - Mientras el segundo se llena se va vaciando el primer buffer.
- *Buffer circular*, más de dos bufers.
      - Adecuado para procesos con ráfagas rápidas de E/S.

*** Caché de buffers de bloques de disco
- Se reserva en espacio de nucleo un espacio de caché de buffers implementado via software.
- Estos caches son copias de bloques de disco recientemente accedidos.
- Se busca evitar el tiempo de acceso E/S a los dispositivos.
- Si no hay espacio en la cache de buffers para el bloque solicitado hay que usar algoritmos de reemplazo.
      - El espacio es limitado.

** Spooling        
- Técnica de control para dispositivos dedicados en entornos multiprogramados.
- Un proceso demonio y un directorio especial (*directorio de spooling*)
      - Un proceso usuario envía los archivos que quiere escribir en el dispositivo E/S al directorio de spooling.
      - El demonio comprueba periodicamente el contenido del directorio
            - Va mandando los archivos al dispositivo de salida.
      - Solo el demonio puede escribir en el dispositivo.
- Se evita un bloqueo excesivo por parte de un proceso del dispositivo de E/S.
- Algunas usos son para impresoras o envío de paquetes por la red.

** Relojes
- En sincronía con el reloj del sistema (cristal de cuarzo)
- Disponen de un contador y algunos registros.
- Cada ciclo decrementan su contador.
      - Cuando el contador de un reloj llega a 0 genera una interrupción.
- Modos de operación
      - *Disparo único*, cuando el contador llega a 0 genera una interrupción.
      - *Disparo de onda cuadrada*, cuando el contador llega a 0 genera una interrupción y se rearma.
            - Se rearma con el valor de alguno de sus registros.
            - *Tic*, tiempo transcurrido entre dos interrupciones.
- Tareas de los relojes en el SO
      - *Planificación de procesos*
      - *Mantenimiento del tiempo real*, un reloc que incrementa.
      - *Disparo de alarmas*, 
      - *Invocación de tareas peródicas del sistema*
...
* Gestión de archivos
** Archivos
- Conjunto de información que se almacena en memoria secundaria, se identifica con un nombre.
      - Directorios
      - Archivo regular
            - Datos, ascii o binarios
            - Programas, binarios
- *Nombre de archivo*, conjunto de carácteres que identifica un archivo.
- *Registos* o *bloques lógicos*, los archivos se forman con un conjunto de bloques de igual o
  distinto tamaño.

*** Atributos  de un archivo
- Información relativa a los archivos que mantiene el SO.
- *Tipo de archivo*
- *Tamaño*
- *Localización*
- *Creador y propietario*
- *Permisos de acceso*
- *Información asociada al tiempo*, cuando fue modificado.

*** Estructura interna de un archivo
- Esrtucturas comunes.
      - *Secuencia de bytes*, bytes contiguos en memoria.
      - *Secuencia de registros*, registros contiguos del mismo tamaño.
            - Cada registro posee una estructura interna.
      - *Registros indexados*, registros de tamaño variable.
            - estructura interna y un identificador del registro.
- *Bloque Físico*, se divide la memoria secundaria en bloques de igual tamaño.
- Los archivos ocupan un conjunto de bloques físicos que no tiene porque ser consecutivo.
- El bloque físico puede contener varios bloques lógicos.

*** Acceso a un archivo
- *Acceso secuencial*, puntero de lectura/escritura.
- *Acceso aleatorio*, Escritura/lectura en cualquier orden.
...
- *Enlazar*
      - los links son entradas en directorios.
- *Desenlazar*
...
- *Estructura de datos con información de bloques libres*
- *Estructura de datos para localizar los bloques asignados a archivos*
- *Área de datos*

*** Asignación de espacio
**** *Asignación continua*
- El archivo contiguo en memoria.
  
**** *Asignación enlazada*
- los bloques físicos tienen la dirección del siguiente bloque físico que pertenece al archivo.
      - Así implementada da problemas al quitar unos pocos bytes de espacio al archivo y dificultar las operaciones con potencias de 2.
- FAT, una tabla situada al principio de la partición.
      - Contiene una entrada por cada bloque físico. y como valor la dirección del siguiente bloque físico.
      - Si es el final del archivo puede contener el valor -1.
      - Si el bloque está vacío puede tener otro valor para indicarlo (0).
      - El problema es tener la fat en memoria.

**** Asignación Indexada
- *Nodo-I*, almacena los atributos de un archivo.
      - Almacena las direcciones físicas de los primeros bloques de un archivo (sin indirección).
...
- El índice del nodo-i asociado a un archivo se almacena en la entrada de directorio.
...
- *Copia de seguridad lógica*, solamente directorios y archivos que el administrador desea guardar.
      ...
      - *Copia Diferencial*, Solamente archivos creados o modificados desde la última copia completa.
      - *Copia incremental*, Solamente archivos creados o modificados desde la última copia incremental.
