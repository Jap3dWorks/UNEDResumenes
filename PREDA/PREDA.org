...
fun Combinar(a,b:entero; i,j:natural; v:vector[1..n] de natural): entero
    ...
    fsi
    si a=-1 && b!=-1 entonces
        dev ComprobarMayoritario(i,j,b,v)
    fsi
    si a!=-1 && b=-1 entonces
        dev ComprobarMayoritario(i,j,a,v)
    fsi    
    si a != -1 && b != -1 entonces
...
*/
fun DistMasCercana(puntos:Puntos, l:entero, r:entero): float
    var
        dl:float
        dr:float
        dc:float
    ...
    sino
        dl <- DistMasCercana(l, r/2)
        dr <- DistMasCercana(r/2 + 1, r)
        dc <- DistMasCercana(r/2, r/2 + 1)     // Distancia entre los puntos más cercanos a la línea de corte.
...
:PROPERTIES:
:ID:       5138539e-f54a-48c5-b96d-abd7ceb3316f
:END:
- Dadas dos cadenas de texto X e Y te tamaño diferente, el sobjetivo es transformar X en Y con el mínimo número de pasos.
- Las posibles operaciones son.
      - Borrar un carácter de X
      - Insertar uno de los carácteres de Y en X
      - Sustituir un carácter de X por uno de Y

#+begin_src pseudocode
tipo Tabla=matriz[0..n,0..m] de entero
fun DistanciaEdicion(X:Vector[1,,n] de caracter, Y:Vector[1..m] de caracter, n,m:entero, C:Tabla)
    var
        i,j,tmp:entero
    fvar
...
fun IdentificarTrans(X:Vector[1..n] de caracter, Y:Vector[1..m] de caracter, n,m:entero, C:Tabla, transforma:Vector[1..nm] de Trans)
    var
        i,j,k:entero
    fvar
    i <- n
...
** Distancia de edición de una cadena de texto
- El problema consiste en transformar la cadena *X* en la cadena *Y* con el mínimo coste posible.
- Cada operación tiene asociada un coste.
      - Borrar un carácter de *X*, con un coste $c_b$.
      - Insertar uno de los carácteres de *Y* en *X*, con un coste $c_i$.
      - Sustituir un carácter de *X* por uno de los de *Y*, con un coste $c_s$.
        
- Este problema también se puede resolver usando [[id:5138539e-f54a-48c5-b96d-abd7ceb3316f][Programación Dinámica]].
...
fun EstimacionOpt(lonX, lonY, k, costeT:enteros): entero
    var
        estimación: entero
    fvar
    si lonX < lonY entonces
        estimación <- costeT + costeInserción * (lonY-lonX)
    sino
        estimación <- costeT + costeBorrado * (lonX-lonY)
    fsi
    dev estimación
ffun
#+end_src
...
#+begin_src pseudocode
fun EstimacionPes(lonX, lonY, k, costeT:entero): entero
    var
        estimación: entero
    fvar
    estimación <- costeT + costeBorrado * (lonX - k)
    estimación <- estimación + costeInserción * (lonY - k)
    dev estimación
ffun
#+end_src
